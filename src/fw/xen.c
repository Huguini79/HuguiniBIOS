...
struct xen_HuguiniBIOS_info {
    char signature[14]; /* XenHVMHuguiniBIOS\0 */
...
static struct xen_HuguiniBIOS_info *validate_info(void)
{
    struct xen_HuguiniBIOS_info *t = (void *)INFO_PHYSICAL_ADDRESS;

    if ( memcmp(t->signature, "XenHVMHuguiniBIOS", 14) ) {
        dprintf(1, "Bad Xen info signature\n");
        return NULL;
    }

    if ( t->length < sizeof(struct xen_HuguiniBIOS_info) ) {
...
{
    struct xen_HuguiniBIOS_info *info = validate_info();
    void **tables;
    int i;
